...
Modified in 2025 to support restoration effects analysis across a range of autoregressive transformer models,
...
def main():
    ...
    for knowledge in tqdm(knowns):
        ...
        print()
        print('normal_num :', normal/3)
        print('fail_num :', fail/3)
        print('correct_num :', (normal/3) -(fail/3))
...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        if model is None:
            ...
            elif model_name == "Qwen/Qwen2.5-1.5B":
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto",
                )
            ...
            elif model_name == "Qwen/Qwen2.5-14B":
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto",
                )
            elif model_name == "microsoft/phi-4":
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto",
                )
            ...
            elif model_name == "ibm-granite/granite-3.2-2b-instruct":
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto",
                )
            elif model_name == "stabilityai/stablelm-zephyr-3b":
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto",
                )
            ...
            if model_name == "meta-llama/Llama-2-7b-hf" or model_name == "meta-llama/Llama-3.1-8B" \
                    or model_name == "deepseek-ai/DeepSeek-R1-Distill-Llama-8B" \
                    or model_name == "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B" \
                    or model_name == "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B" \
                    or model_name == "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B" \
                    or model_name == "deepseek-ai/DeepSeek-R1-Distill-Qwen-14B" \
                    or model_name == "meta-llama/Llama-3.2-1B" \
                    or model_name == "meta-llama/Llama-3.2-3B" \
                    or model_name == "EleutherAI/gpt-j-6B" \
                    or model_name == "Qwen/Qwen2.5-0.5B" \
                    or model_name == "Qwen/Qwen2.5-1.5B" \
                    or model_name == "Qwen/Qwen2.5-3B" \
                    or model_name == "Qwen/Qwen2.5-7B" \
                    or model_name == "Qwen/Qwen2.5-14B":
                model.eval()
            else:
                model.eval().cuda()
        ...
        self.layer_names = [
            ...
            if (re.match(r"^(transformer|gpt_neox)\.(h|layers)\.\d+$|^model\.layers\.\d+$", n))
...
# Experiment with Multiple Autoregressive Transformer models
...
def make_inputs(tokenizer, prompts, device="cuda"):
    token_lists = [tokenizer.encode(p,add_special_tokens=False) for p in prompts] #add_special False
...
def find_token_range(tokenizer, token_array, substring):
    ...
    print('1. whole string ', whole_string)
    print('2. substring ', substring)
...
#     print("1. toks : ", toks)
#     print('2. substring : ', substring)
#     clean_toks = [t.strip() for t in toks]  # 공백 제거해서 비교
#
#     substring = substring.strip()
#     sub_words = substring.split()  # 예: ['Beats', 'Music']
#
#     for i in range(len(clean_toks)):
#         for j in range(i + 1, len(clean_toks) + 1):
#             window = "".join(clean_toks[i:j]).replace(" ", "")
#             target = "".join(sub_words)
#             if window == target:
#                 print('i : ', i , 'j : ', j)
#                 return (i, j)
#     print("Substring not found in tokenized string")
#     return None, None
